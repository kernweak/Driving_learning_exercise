	...
	IN BOOLEAN         bIsDir)   // 是否为文件夹
{
	...
	ULONG ulAttributes =
		OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE;//不区分大小写|该句柄只能在内核模式下用
...
void GetReLoadBuf(PUNICODE_STRING KerPath, PCHAR* pReloadBuf)
{
	...
	*pReloadBuf = ExAllocatePool(NonPagedPool, pNt->OptionalHeader.SizeOfImage);//内存
	...
	{
		RtlCopyMemory(
			*pReloadBuf + pSection[i].VirtualAddress,//内存的偏移
			pKernelBuf + pSection[i].PointerToRawData,//文件中的偏移
